"""
Test Data Manager Page Object for Insider Test Automation Project
...
class DataManager(BasePage):
    ...
    def __init__(self, driver):
        """
        Initialize TestDataManager with WebDriver instance.
    ...
    def validate_cross_browser_compatibility(self, popup_page, browser="chrome"):
        """
        Validate cross-browser compatibility for popup functionality.
        ...
        results = {
            ...
            "compatible": True,
            "issues": [],
            "popup_functional": False,
            "details": {}
        }

        try:
            # Test basic popup trigger
            if popup_page.click_show_instantly_button():
                results["details"]["trigger_success"] = True

                # Wait for popup
                if popup_page.wait_for_popup_to_appear(timeout=10):
                    results["popup_functional"] = True
                    results["details"]["popup_appeared"] = True

                    # Test popup visibility
                    if popup_page.is_popup_visible():
                        results["details"]["popup_visible"] = True
                    else:
                        results["compatible"] = False
                        results["issues"].append("Popup not visible after appearing")
                else:
                    results["compatible"] = False
                    results["issues"].append("Popup did not appear")
            else:
                results["compatible"] = False
                results["issues"].append("Could not trigger popup")

            # Browser-specific checks
            if browser.lower() == "firefox":
                # Firefox-specific issues
                if not results["popup_functional"]:
                    results["issues"].append("Firefox compatibility issue - popup not functional")
            elif browser.lower() == "safari":
                # Safari-specific issues
                if not results["popup_functional"]:
                    results["issues"].append("Safari compatibility issue - popup not functional")

        except Exception as e:
            results["compatible"] = False
            results["issues"].append(f"Cross-browser test failed: {str(e)}")

        return results

    def validate_mobile_functionality(self, popup_page, device_type="mobile"):
        """
        Validate mobile/tablet functionality for popup.
        ...
        Returns:
            Dictionary with mobile validation results
        """
        results = {
        ...
        try:
            # Test popup trigger on mobile/tablet
            if popup_page.click_show_instantly_button():
                results["details"]["trigger_success"] = True

                # Wait for popup with extended timeout for mobile
                if popup_page.wait_for_popup_to_appear(timeout=15):
                    results["popup_functional"] = True
                    results["details"]["popup_appeared"] = True
                    ...
                    if popup_page.is_popup_visible():
                        ...
                        if popup_page.click_close_button():
                            results["details"]["close_functional"] = True
                        else:
                            results["functional"] = False
                            results["issues"].append("Close button not functional on mobile")
                    else:
                        results["functional"] = False
                        results["issues"].append("Popup not visible on mobile")
                else:
                    results["functional"] = False
                    results["issues"].append(f"Popup did not appear on {device_type}")
            else:
                results["functional"] = False
                results["issues"].append(f"Could not trigger popup on {device_type}")

        except Exception as e:
            results["functional"] = False
            results["issues"].append(f"Mobile test failed: {str(e)}")
    ...
    def _get_cart_count(self):
        ...
        cart_locators = [
            ...
            (By.XPATH, "//span[contains(@class, 'cart')]"),
        ...
        for locator in cart_locators:
            try:
                ...
                self.logger.warning(f"Error getting cart count with locator {locator}: {e}")
                continue
